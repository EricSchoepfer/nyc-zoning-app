document.getElementById("searchBtn").addEventListener("click", async function() {
    ...
    // Direct baseline default variables from Version 1/2 blueprints
    ...
    try {
        // Query the live NYC Planning Labs Geosearch endpoint
        var url = "https://planninglabs.nyc" + encodeURIComponent(rawAddress);
        var res = await fetch(url);
        var data = await res.json();

        // Version 1 & 2 layout parsing fix: Target properties list inside features index 0 array
        if (data && data.features && data.features.length > 0) {
            ...
            console.warn("No API features returned. Displaying baseline architectural defaults.");
        }
    } catch (err) {
        console.error("API link timed out. Rendering baseline parameter configuration card:", err.message);
    }

    // Pass items smoothly to renderer method
...
function buildMatrixLayout(title, district, overlay, special, bbl) {
    ...
    // 1. Permitted Elements Panel Mapper
    ...
    // 2. Special District Overrides Notice Configuration
    ...
    // Reveal hidden workspace card wrappers
